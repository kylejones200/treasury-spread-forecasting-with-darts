...
def fetch_fred_data(series_id: str, start_date: str='2000-01-01', end_date: Optional[str]=None) -> TimeSeries:
...
def fit_auto_arima(series: TimeSeries, forecast_horizon: int=30, num_samples: int=1000) -> Tuple[AutoARIMA, TimeSeries]:
...
def evaluate_forecast(actual: TimeSeries, forecast: TimeSeries) -> Dict[str, float]:
    ...
    return {'mape': mape_score, 'mse': mse_score}
